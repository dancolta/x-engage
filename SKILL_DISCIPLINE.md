...
- **What specific Dan-rejected draft does this prevent?**
- **What specific Dan-approved draft does this enable?**
...
Examples that would fail: "AI overuses 'em-dash' so ban semicolons too." No — wait until Dan flags an actual semicolon-using draft.
...
The voice profile should say what Dan DOES, not what he doesn't. The lint catches what slips through.

### 3. New examples go in the corpus, not the prompt

When a new POV, receipt, or voice example appears (Dan's voice memo, a new shipped repo, a new flagged "this is me" draft), the move is:

- **DO:** add it to `dan-x-corpus.md` as a tagged entry. The retrieval picks it up at draft-time when relevant.
- **DON'T:** add it to the prompt as "Dan's perspective on X."
...
## When Dan flags a draft as "not my voice"
...
4. **Test the lint** — run a smoke test against the rejected draft + a clean Dan-shape variant to confirm no false positives.
...
## When Dan flags a draft as "yes, this is me"

The protocol:

1. **Run `/x-engage good <id>`** — pins the draft into `good-drafts.md` as a vibe reference (handled by existing code).
2. **Also consider adding to `dan-x-corpus.md`** if the draft demonstrates a new pattern not already covered (new register, new POV, new receipt shape). Manual edit, tagged like the seed entries.
...
- Tilde-frequency quota (Dan doesn't use tildes — period)
...
- 3-noun-phrase listicle bans (false positives on legitimate Dan tricolons)
- Comma-before-and ban (Dan's real corpus has both forms)
...
- Corpus grows to 30+ Dan-voiced entries
- Drafts feel like Dan typed them, not like an AI wearing a hoodie
